...
bool Acpi::init()
{
    ...
    if(!root)
    {
        Gpu::print_error("ACPI: No root table\n");
        return false;
    }
    ...
    if(xsdt)
    {
        Gpu::print("Using XSDT\n");
    }
    else
    {
        Gpu::print("Using RSDT\n");
    }
    ...
    if(!facp)
    {
        Gpu::print_error("ACPI: FACP not found\n");
        return false;
    }

    Gpu::print("ACPI: FACP found\n");
    ...
    if(dsdt)
    {
        ...
        {
            Gpu::print("ACPI: _S5 found in DSDT\n");
            return true;
        }
    }
    ...
    for(uint32_t i = 0; i < entries; i++)
    {
        ...
        if(acpi_sig(table->Signature, "SSDT"))
        {
            ...
            {
                Gpu::print("ACPI: _S5 found in SSDT\n");
                return true;
            }
        }
    }

    Gpu::print_error("ACPI: _S5 not found\n");
...
static bool acpi_enable()
{
    if(PM1a_CNT == 0)
    {
        Gpu::print_error("ACPI: PM1a_CNT missing\n");
        return false;
    }
    ...
    {
        Gpu::print_error("ACPI: Cannot enable ACPI\n");
        return false;
    }

    Gpu::print("ACPI: Enabling ACPI mode\n");
    ...
    Gpu::print_error("ACPI: Enable timeout\n");
...
void Acpi::shutdown()
{
    Gpu::print_warn("NasuaOS: Shutting down\n");

    if(!Acpi::init())
    {
        Gpu::print_error("ACPI: initialization failed\n");
    ...
    if(!acpi_enable())
    {
        Gpu::print_error("ACPI: cannot enable\n");
        return;
    }
    ...
    Gpu::print("ACPI: Sending sleep command\n");
...
void Acpi::reboot()
{
    Gpu::print_warn("NasuaOS: Restarting\n");
